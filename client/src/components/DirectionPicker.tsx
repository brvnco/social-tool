        ...
        {directions.map((d, i) => {
          ...
          return (
            <button
              ...
              onClick={() => pick(d)}
              disabled={picking}
              className="w-full text-left bg-delta-card rounded-3xl shadow-card border border-delta-border p-6 hover:shadow-card-hover hover:border-delta-accent/30 transition-all group disabled:opacity-50"
            >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2.5 mb-3">
                    ...
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map(n => (
                        <span key={n} className={`w-2 h-2 rounded-full ${n <= d.score ? 'bg-delta-accent' : 'bg-gray-200 dark:bg-gray-700'}`} />
                      ))}
                    </div>
                  </div>
                  <h3 className="font-bold text-lg text-delta-text group-hover:text-delta-accent transition">
                    {d.topic}
                  </h3>
